package org.jze.common.redis.utils;

import lombok.AccessLevel;
import lombok.NoArgsConstructor;
import org.jze.common.core.utils.SpringUtils;
...
public class QueueUtils {
     ...
     * @param time      延迟时间
     */
    public static <T> void addDjzeedQueueObject(String queueName, T data, long time) {
        addDjzeedQueueObject(queueName, data, time, TimeUnit.MILLISECONDS);
    }
     ...
     * @param timeUnit  单位
     */
    public static <T> void addDjzeedQueueObject(String queueName, T data, long time, TimeUnit timeUnit) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDjzeedQueue<T> djzeedQueue = CLIENT.getDjzeedQueue(queue);
        djzeedQueue.offer(data, time, timeUnit);
    }
     ...
     * @param queueName 队列名
     */
    public static <T> T getDjzeedQueueObject(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDjzeedQueue<T> djzeedQueue = CLIENT.getDjzeedQueue(queue);
        return djzeedQueue.poll();
    }

    /**
     * 删除延迟队列数据
     */
    public static <T> boolean removeDjzeedQueueObject(String queueName, T data) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDjzeedQueue<T> djzeedQueue = CLIENT.getDjzeedQueue(queue);
        return djzeedQueue.remove(data);
    }

    /**
     * 销毁延迟队列 所有阻塞监听 报错
     */
    public static <T> void destroyDjzeedQueue(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDjzeedQueue<T> djzeedQueue = CLIENT.getDjzeedQueue(queue);
        djzeedQueue.destroy();
    }
     ...
     */
    public static <T> void subscribeBlockingQueue(String queueName, Function<T, CompletionStage<Void>> consumer, boolean isDjzeed) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        if (isDjzeed) {
            // 订阅延迟队列
            CLIENT.getDjzeedQueue(queue);
        }
